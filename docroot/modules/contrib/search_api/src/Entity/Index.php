...
class Index extends ConfigEntityBase implements IndexInterface {
  ...
  public function indexItems($limit = '-1', $datasource_id = NULL) {
    if ($this->hasValidTracker() && !$this->isReadOnly()) {
      ...
      \Drupal::service('logger.channel.search_api')->info('Loading $next_set...');
      $next_set = $tracker->getRemainingItems($limit, $datasource_id);
      \Drupal::service('logger.channel.search_api')->info('$next_set loaded...');
      if (!$next_set) {
        return 0;
      }
      \Drupal::service('logger.channel.search_api')->info('Loading individual items of next set...');
      $items = $this->loadItemsMultiple($next_set);
      \Drupal::service('logger.channel.search_api')->info('Individual items loaded');
      if (!$items) {
        return 0;
      }
      try {
